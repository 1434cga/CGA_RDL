...
$tttime = $Hour * 3600 + $Minute * 60 + $Second;
	...
	while ($in = <STCI_INPUT>) {
		if($in =~ /^\s*###/){
			...
			$stcI_fileprefix = $1;
		} elsif($in =~ /^stcI_MACRO_START\s*\:\s*(.*)\s*/){
			print_fp("MACRO START : $1\n",DBG,STDOUT);
			$stcI_macro_start = 1;
			$macro_def = $1;
			$macro_def =~ /^\s*([^\s\(]+)\s*\(\s*([^\)]+)\s*\)/;
			print_fp("MACRO START : $1 $2\n",DBG,STDOUT);
			$stcI_macro_name = $1;
			$stcI_macro_arg = $2;
			@stcI_marg = split(",",$stcI_macro_arg);
			print_fp("MACRO MARG : @stcI_marg\n",DBG,STDOUT);
			my $myargcnt = 0;
			foreach my $myarg (@stcI_marg){
				$myarg =~ /\s*(\S+)\s*/;
				print_fp("MACRO START arg $myargcnt : $1\n",DBG,STDOUT);
				$macro{$stcI_macro_name}{param}{$myargcnt}{name} = $1;
				$myargcnt++;
			}
			$macro{$stcI_macro_name}{paramcount} = $myargcnt;
			$macro{$stcI_macro_name}{content} = "";
		} elsif($in =~ /^stcI_MACRO_END/){
	...
	foreach $mym (keys %macro){
		print_fp("MACRO NAME : $mym\n",DBG,STDOUT);
		print_fp("MACRO content : $macro{$mym}{content}\n",DBG,STDOUT);
		print_fp("MACRO arg count : $macro{$mym}{paramcount}\n",DBG,STDOUT);
	...
	print_fp("stcI_FILEBODY : $stcI_filebody\n",STDOUT,DBG);
	print_fp("stcI_FILE_LOWER : $stcI_file_lower\n",STDOUT,DBG);
	print_fp("stcI_ALL_LOWER : $stcI_all_lower\n",STDOUT,DBG);
	...
	foreach $temp (keys %$stcI_for){
		...
		print_fp("FileName : $stcfilename\.$stcI_extension\n",DBG,STDERR);
		print_fp("Temporary stcI FileName : $tmpStcIFileName\n",DBG,STDERR);
		@stcI_lines = split("\n",$stcI_lines);
					...
					foreach my $myarg (@marg){
						...
						if($myarg ne ""){
							if($myarg eq "stcI_empty"){
								$my_macro_content =~ s/$macro{$macro_name}{param}{$mycnt}{name}//g;
							} else {
								$my_macro_content =~ s/$macro{$macro_name}{param}{$mycnt}{name}/$myarg/g;
							}
						}
						$mycnt++;
					}
					print STCI_OUTPUT "$my_macro_content\n";
					if($macro{$macro_name}{paramcount} != $mycnt){
						print_fp("ERROR : $macro_name argument's count is differnet : $macro{$macro_name}{paramcount} $mycnt : $macro_name\n",STDERR,DBG);
